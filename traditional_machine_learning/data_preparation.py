...
def split_data(
    ...
    train, val_temp, y_train, y_val_temp = train_test_split(
        features_df, target_df, test_size=first_split_proportion, random_state=seed
    )
    val, test, y_val, y_test = train_test_split(
        val_temp, y_val_temp, test_size=second_split_proportion, random_state=seed
    )
...
def preprocess_data(
    ...
    with mlflow.start_run(run_name="preprocessor"):
        ...
        cat_transformer = Pipeline(steps=[("ohe", ohe)])

        num_transformer = Pipeline(steps=[("scaler", scaler)])

        hc_transformer = Pipeline(steps=[("loo", loo_encoder), ("scaler", scaler)])
        ...
        cat_feature_names = list(
            preprocessor.transformers_[0][1].get_feature_names_out()
        )
...
def id_highly_correlated_variables(
    ...
    corr_df.rename(
        columns={"index": "variable1", "variable": "variable2", "value": "corr"},
        inplace=True,
    )
...
def remove_highly_correlated_vars(
    ...
    corr_mi_score_df = high_corr_df.merge(
        mi_score_df, left_on="variable1", right_on="features", how="left"
    )
    ...
    corr_mi_score_df = corr_mi_score_df.merge(
        mi_score_df, left_on="variable2", right_on="features", how="left"
    )
    ...
    corr_mi_score_df["variable_to_drop"] = corr_mi_score_df.apply(
        lambda row: row["variable2"]
        if (row["mi_score_variable1"] > row["mi_score_variable2"])
        else row["variable1"],
        axis=1,
    )
